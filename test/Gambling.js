...
describe("Gambling Contract", function () {
    ...
    it("A user cannot participate with zero GMB token", async function () {
        await expect(gambling.connect(lp).participate(0, 1, pairAddress, false)).to.be.reverted;
    });
    ...
    it("A user cannot participate with less GMB token in his/her account than the one mentioned for participation", async function () {
        await expect(gambling.connect(lp).participate(1, 1, pairAddress, false)).to.be.reverted;
    });
    ...
    it("A user can participate if everything is ok", async function () {
        ...
        await gambling.connect(lp).participate(1, 1, pairAddress, false)
        await mineBlocks(2)
    });
    ...
    it("A user cannot participate more than once", async function () {
        ...
        await claimFromLP();
        await mineBlocks(2)
        await expect(gambling.connect(lp).participate(1, 1, pairAddress, false)).not.to.be.reverted;
        await expect(gambling.connect(lp).participate(1, 1, pairAddress, false)).to.be.reverted;
    });
    ...
    it("GMB token decrease from user account after participation", async function () {
        ...
        await gambling.connect(lp).participate("100000000000000000000", 100, pairAddress, false); // Consider the force claim
    ...
    it("Only admin can call endGame", async function () {
        ...
        await claimFromLP();
        await mineBlocks(2)
        await gambling.connect(lp).participate(10, 1, pairAddress, false);
        await mineBlocks(40000)
    ...
    it("endTurn should work if everything is fine", async function () {
        ...
        await gambling.connect(lp).participate("100000000000000000000", 1, pairAddress, false);
    ...
    it("end to end one round game", async function () {
        ...
        await gambling.connect(_user).participate("20000000000000000000", 40, pairAddress, false);

        await mineBlocks(2)
        // await gambling.connect(lp).participate(10, 1);
        await claimFromLP(pairAddress, _lpAddress)
        await gambling.connect(lp).participate("20000000000000000000", 1, pairAddress, false);
        ...
        expect(await gmb.balanceOf(_user.address)).to.equal("59600000000000000000")
    ...
    it("end to end three rounds game", async function () {
        ...
        // Round one
        await gambling.connect(_user).participate(amount, 40, pairAddress, false);
        await gambling.connect(lp).participate(amount, 1, pairAddress, false);
        ...
        expect(await gmb.balanceOf(_lpAddress)).to.equal("90239974999999899880025")
        expect(await pair.balanceOf(_user.address)).to.equal("999999999999999")
        expect(await pair.balanceOf(_lpAddress)).to.equal("1998999999999999001")
        ...
        expect(await gmb.balanceOf(_lpAddress)).to.equal("80239974999999899880025")
        expect(await pair.balanceOf(_user.address)).to.equal("999999999999999")
        expect(await pair.balanceOf(_lpAddress)).to.equal("1998999999999999001")

        // Round three
        await gambling.connect(_user).participate(amount, 75000001, pairAddress, false);
        await gambling.connect(lp).participate(amount, 1, pairAddress, false);
        ...
        expect(await gmb.balanceOf(_user.address)).to.equal("114800000000000000000000")
        expect(await gmb.balanceOf(_lpAddress)).to.equal("70239974999999899880025")
        expect(await pair.balanceOf(_user.address)).to.equal("999999999999999")
        expect(await pair.balanceOf(_lpAddress)).to.equal("1998999999999999001")
    })
    ...
    it("end to end five rounds game", async function () {
        ...
        expect(await gmb.balanceOf(_lpAddress)).to.equal("60239974999999899880025")
        expect(await pair.balanceOf(_user.address)).to.equal("999999999999999")
        expect(await pair.balanceOf(_lpAddress)).to.equal("1998999999999999001")
        ...
        expect(await gmb.balanceOf(_user.address)).to.equal("64800000000000000000000")
        expect(await gmb.balanceOf(_lpAddress)).to.equal("50239974999999899880025")
        expect(await pair.balanceOf(_user.address)).to.equal("999999999999999")
        expect(await pair.balanceOf(_lpAddress)).to.equal("1998999999999999001")
    })
    ...
    it("end to end five rounds game with lending", async function () {
        ...
        await gambling.connect(_user).participate(amount, 40, pairAddress, true);
        await gambling.connect(lp).participate(amount, 1, pairAddress, false);
        await gambling.emergencyEnd(626000000);
        await expect(gambling.connect(_user).claimPrize(3)).to.be.reverted
        await expect(gambling.connect(lp).claimPrize(3)).to.be.reverted
        await gambling.connect(lp).claimLP(3)
        await gambling.connect(_user).claimLP(3)
        expect(await gmb.balanceOf(_user.address)).to.equal("70030177999999999999900")
        expect(await gmb.balanceOf(_lpAddress)).to.equal("70429977999999899805100")
        expect(await pair.balanceOf(_user.address)).to.equal("0")
        expect(await pair.balanceOf(_lpAddress)).to.equal("1000499999999999500")

        // Round four
        await expect(gambling.connect(_user).participate(amount, 40, pairAddress, false)).to.be.reverted
        await gambling.connect(_user).participate(amount, 40, pairAddress, true);
        await gambling.connect(lp).participate(amount, 1, pairAddress, false);
        await gambling.emergencyEnd(626000000);
        await expect(gambling.connect(_user).claimPrize(4)).to.be.reverted
        ...
        await gambling.connect(lp).claimLP(4)
        await gambling.connect(_user).claimLP(4)
        expect(await gmb.balanceOf(_user.address)).to.equal("60020177999999999999900")
        expect(await gmb.balanceOf(_lpAddress)).to.equal("60429977999999899805100")
        expect(await pair.balanceOf(_user.address)).to.equal("0")
        expect(await pair.balanceOf(_lpAddress)).to.equal("1000499999999999500")

        // Round five
        await gambling.connect(_user).participate(amount, 75000001, pairAddress, true);
        await gambling.connect(lp).participate(amount, 1, pairAddress, false);
        await gambling.emergencyEnd(700000000);
        await gambling.connect(_user).claimPrize(5)
        await expect(gambling.connect(lp).claimPrize(5)).to.be.reverted
        await gambling.connect(lp).claimLP(5)
        await expect(gambling.connect(_user).claimLP(5)).to.be.reverted
        expect(await gmb.balanceOf(_user.address)).to.equal("64810177999999999999900")
        expect(await gmb.balanceOf(_lpAddress)).to.equal("50429977999999899805100")
        expect(await pair.balanceOf(_user.address)).to.equal("0")
        expect(await pair.balanceOf(_lpAddress)).to.equal("1000499999999999500")
        await lending.connect(lp).exitLendingPool(1)
        expect(await gmb.balanceOf(_lpAddress)).to.equal("50459977999999897915092")
